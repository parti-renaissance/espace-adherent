...
sudo /opt/google-cloud-sdk/bin/kubectl set image deploy/staging-migrate enmarche=eu.gcr.io/$GCLOUD_PROJECT/app:$CIRCLE_SHA1
...
migration_log=$(sudo /opt/google-cloud-sdk/bin/kubectl logs deploy/staging-migrate --container=enmarche || true)
