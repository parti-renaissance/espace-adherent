...
db-init: vendor wait-for-db                                                                            ## Init the database
	...
	$(DOCKER_COMPOSE) exec -T db mysql -uroot -proot --quick enmarche < ./dump/dump-2023.sql
...
tfp-db-init: wait-for-db                                                                                    ## Init databases for tests
	...
	$(DOCKER_COMPOSE) exec -T db mysql -uroot -proot --quick enmarche_test < ./dump/dump-2023.sql
