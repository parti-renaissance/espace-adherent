...
Feature:
    ...
    Scenario: As a non logged-in user I cannot get activity histories
        When I send a "GET" request to "/api/v3/user_activity_histories"
        Then the response status code should be 401

    Scenario: As a logged-in user I can get my activity history paginated and ordered by occurredAt DESC
        Given I am logged with "carl999@example.fr" via OAuth client "JeMengage Mobile" with scope "jemarche_app"
        When I send a "GET" request to "/api/v3/user_activity_histories?adherent.uuid=e6977a4d-2646-5f6c-9c82-88e58dca8458"
    ...
    Scenario: As a logged-in user filtering on a UUID with no activity returns an empty list
        Given I am logged with "carl999@example.fr" via OAuth client "JeMengage Mobile" with scope "jemarche_app"
        When I send a "GET" request to "/api/v3/user_activity_histories?adherent.uuid=a046adbe-9c7b-56a9-a676-6151a6785dda"
        Then the response status code should be 200
