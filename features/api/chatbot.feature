...
Feature:
    ...
    Scenario: As a non logged-in user I can not list thread messages
        Given I send a "GET" request to "/api/v3/ai/threads/a046adbe-57d6-4115-91bf-e8e68ef1e0fa/messages"
        Then the response status code should be 401

    Scenario: As a logged-in user I can not list another user's thread messages
        Given I am logged with "jacques.picard@en-marche.fr" via OAuth client "JeMengage Web"
        When I send a "GET" request to "/api/v3/ai/threads/a046adbe-57d6-4115-91bf-e8e68ef1e0fa/messages"
    ...
    Scenario: As a canary tester I can access the second page of messages
        Given I am logged with "president-ad@renaissance-dev.fr" via OAuth client "JeMengage Web"
        When I send a "GET" request to "/api/v3/ai/threads/a046adbe-57d6-4115-91bf-e8e68ef1e0fa/messages?page=2"
        Then the response status code should be 200
        And the response should be in JSON
        And the JSON should be a superset of:
            """
    ...
    Scenario: As a canary tester I can not list messages of another user's thread
        Given I am logged with "president-ad@renaissance-dev.fr" via OAuth client "JeMengage Web"
        When I send a "GET" request to "/api/v3/ai/threads/b157bfcf-68e7-5226-a2c0-f9f79f020fbb/messages"
        Then the response status code should be 200
        And the response should be in JSON
        And the JSON should be a superset of:
            """
    ...
    Scenario: As the owner of a chatbot thread I can not list its messages
    ...
    Scenario: As a canary tester requesting a non-existent thread I get an empty result
        Given I am logged with "president-ad@renaissance-dev.fr" via OAuth client "JeMengage Web"
        When I send a "GET" request to "/api/v3/ai/threads/00000000-0000-0000-0000-000000000000/messages"
        Then the response status code should be 200
        And the JSON node "items" should have 0 elements
