Feature:
  ...
  Scenario Outline: Publish message on user created|updated
    ...
    And "api_sync" should have message below:
      | routing_key   | body                                                                                                                                                                                                                                |
      | <routing_key> | {"uuid":"313bd28f-efc8-57c9-8ab7-2106c8be9697","subscriptionExternalIds":"@array@.count(2).inArray('123abc').inArray('456def')","city":"Zürich","country":"CH","zipCode":"8057","tags":["CH"],"emailAddress":"michelle.dufour@example.ch","firstName":"Michelle","lastName":"Dufour"}  |
    Examples:
      | event        | routing_key  |
  ...
  Scenario Outline: Publish message on committee created|updated
    ...
    And "api_sync" should have message below:
      | routing_key   | body                                                                                                                                                                                                                                                                                                    |
      | <routing_key> | {"uuid":"515a56c0-bde8-56ef-b90c-4745b1c93818","name":"En Marche Paris 8","slug":"en-marche-paris-8","status":"APPROVED","membersCount":4,"tags":"@array@.count(2).inArray('75').inArray('75008')","longitude":2.3132432,"latitude":48.8705073,"country":"FR","address":"60 avenue des Champs-Élysées","zipCode":"75008","city":"Paris 8e"}  |
    Examples:
      | event             | routing_key       |
  ...
  Scenario Outline: Publish message on citizen project created|updated
    ...
    And "api_sync" should have message below:
      | routing_key   | body                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
      | <routing_key> | {"uuid":"aa364092-3999-4102-930c-f711ef971195","status":"APPROVED","membersCount":4,"name":"Le projet citoyen \u00e0 Paris 8","slug":"75008-le-projet-citoyen-a-paris-8","category":"Nature et Environnement","longitude":2.3032432,"latitude":48.8705073,"country":"FR","address":"60 avenue des Champs-\u00c9lys\u00e9es","zipCode":"75008","city":"Paris 8e","tags":"@array@.count(2).inArray('75').inArray('75008')","subtitle":"Le projet citoyen des habitants du 8\u00e8me arrondissement de Paris.","district":"Paris 8e","author":"Jacques P.","thumbnail":"http:\/\/test.enmarche.code\/assets\/images\/citizen_projects\/default.png"} |
    Examples:
      | event                   | routing_key             |
