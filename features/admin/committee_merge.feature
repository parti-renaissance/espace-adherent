...
Feature: Merge committees from admin panel
  ...
  Scenario: A committee merge must trigger events in RabbitMQ
    ...
    And I press "Fusionner"
    Then the response status code should be 200
    And I should see "l'arrivée de 1 nouveaux membres au sein du comité de destination En Marche Paris 8 (1)"
    And "api_sync" should have 0 message
    ...
    And "api_sync" should have messages below:
      | routing_key       | body                                                                                                                                                                                                                                                                                                                 |
      | committee.updated | {"uuid":"b0cd0e52-a5a4-410b-bba3-37afdd326a0a","status":"REFUSED","membersCount":2,"name":"En Marche Dammarie-les-Lys","slug":"en-marche-dammarie-les-lys","tags":["77"],"longitude":"2.622016","latitude":"48.518219","country":"FR","address":"824 Avenue du Lys","zipCode":"77190","city":"Dammarie-les-Lys"}     |
