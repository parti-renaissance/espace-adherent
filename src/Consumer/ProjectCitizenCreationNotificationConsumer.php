...
class ProjectCitizenCreationNotificationConsumer extends AbstractConsumer
{
    ...
    protected function doExecute(array $data): int
    {
        ...
        if (null === $citizenProject) {
            $this->writeln('citizen project not found', 'citizen project with '.$data['uuid']. ' not found');
        ...
        $this->writeln('info', sprintf('Start sending. offset : %s | totalAdherent : %s | citizenProjectUuid', $offset, $totalAdherent, $citizenProject->getUuid()->toString()));
        ...
        try {
            do {
                ...
                foreach ($adherents as $adherent) {
                    $this->citizenProjectMessageNotifier->sendAdherentNotificationCreation($adherent);
                }
            ...
            $this->getLogger()->info('Message correctly send from offset '.$data['offset'].' to the end');
