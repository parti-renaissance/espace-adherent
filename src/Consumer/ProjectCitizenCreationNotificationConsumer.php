...
class ProjectCitizenCreationNotificationConsumer extends AbstractMailerConsumer
{
    /**
    ...
    protected function doExecute(array $data): int
    {
        ...
        if (!$totalAdherent) {
            $this->writeln('info', 'No adherent to notify foud for '.$data['uuid'].' citizen project');
        ...
        try {
            do {
                ...
                foreach ($adherents as $adherent) {
                    $this->citizenProjectMessageNotifier->sendAdherentNotificationCreation($adherent, $citizenProject);
                }
