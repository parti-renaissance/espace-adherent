...
class ElectionRepository extends ServiceEntityRepository
{
    ...
    public function getElectionsToClose(\DateTime $date, int $notification): array
    {
        return $this->createQueryBuilder('election')
            ...
            ->setParameters(new ArrayCollection([
                new Parameter('start_date', $date),
                new Parameter('end_date', (clone $date)->modify('+1 '.(Designation::NOTIFICATION_VOTE_REMINDER_1H === $notification ? 'hour' : 'day'))),
                new Parameter('open', ElectionStatusEnum::OPEN),
                new Parameter('notification', $notification),
            ]))
            ->getQuery()
            ->getResult()
        ;
    ...
    public function findIncomingElections(\DateTime $date)
    {
        return $this->createQueryBuilder('election')
            ...
            ->setParameters(new ArrayCollection([
                new Parameter('start_date', $date),
                new Parameter('end_date', (clone $date)->modify('+2 days')),
                new Parameter('open', ElectionStatusEnum::OPEN),
                new Parameter('notification', Designation::NOTIFICATION_VOTE_ANNOUNCEMENT),
            ]))
            ->getQuery()
            ->getResult()
        ;
    ...
    public function getElectionStats(Designation $designation): array
    {
        return $this->createQueryBuilder('el')
            ...
            ->setMaxResults(1)
            ->setParameters(new ArrayCollection([
                new Parameter('designation_id', $designation->getId()),
            ]))
            ->getQuery()
            ->getSingleResult()
        ;
