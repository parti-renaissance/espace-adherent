...
class AppSessionRepository extends ServiceEntityRepository
{
    ...
    public function terminateStaleSessions(): void
    {
        $this->createQueryBuilder('s')
            ...
            ->setParameter('date', new \DateTimeImmutable()->modify(\sprintf('-%d months', AppSession::ACTIVE_DURATION_IN_MONTHS)))
            ->getQuery()
            ->execute()
        ;
    }
    ...
    public function findByPushToken(string $pushToken): ?AppSession
    {
        return $this->createQueryBuilder('s')
            ...
            ->innerJoin('link.pushToken', 'token', Join::WITH, 'token.identifier = :push_token')
