...
class ReferralRepository extends ServiceEntityRepository
{
    ...
    public function isEmailReported(string $email): bool
    {
        return 0 < $this->createQueryBuilder('referral')
            ...
            ->setParameters(new ArrayCollection([
                new Parameter('hash', Referral::createHash($email)),
                new Parameter('status_reported', StatusEnum::REPORTED),
            ]))
            ->getQuery()
    ...
    public function finishReferralAdhesionStatus(Adherent $adherent): void
    {
        $this->createQueryBuilder('r')
            ...
            ->setParameters(new ArrayCollection([
                new Parameter('new_status', StatusEnum::ADHESION_FINISHED),
                new Parameter('date', new \DateTimeImmutable()),
                new Parameter('status_to_update', [StatusEnum::ACCOUNT_CREATED, StatusEnum::INVITATION_SENT]),
                new Parameter('referred', $adherent),
            ]))
            ->getQuery()
            ->execute()
        ;
    ...
    public function updateReferralsStatus(Adherent $adherent, ?Referral $excludeReferral, StatusEnum $status): void
    {
        $qb = $this->createQueryBuilder('r')
            ...
            ->setParameters(new ArrayCollection([
                new Parameter('new_status', $status),
                new Parameter('date', new \DateTimeImmutable()),
                new Parameter('status_to_update', StatusEnum::INVITATION_SENT),
    ...
    public function getStatistics(Adherent $referrer): array
    {
        return $this->createQueryBuilder('referral')
            ...
            ->setParameters(new ArrayCollection([
                new Parameter('status_finished', StatusEnum::ADHESION_FINISHED->value),
                new Parameter('status_reported', StatusEnum::REPORTED->value),
                new Parameter('referrer', $referrer),
            ]))
            ->getQuery()
            ->getSingleResult()
        ;
    ...
    public function findFinishedForAdherent(Adherent $adherent): ?Referral
    {
        return $this->createQueryBuilder('referral')
            ...
            ->setParameters(new ArrayCollection([
                new Parameter('status_finished', StatusEnum::ADHESION_FINISHED),
                new Parameter('referred', $adherent),
            ]))
            ->getQuery()
            ->setMaxResults(1)
