...
class CommitteeRepository extends ServiceEntityRepository
{
    ...
    public function findApprovedByAddress(Address $address, ?int $limit = null): array
    {
        return $this->createQueryBuilder('c')
            ...
            ->setParameters(new ArrayCollection([
                new Parameter('address', $address->getAddress()),
                new Parameter('postal_code', $address->getPostalCode()),
                new Parameter('city_name', $address->getCityName().'%'),
                new Parameter('country', $address->getCountry()),
                new Parameter('approved', Committee::APPROVED),
            ]))
            ->setMaxResults($limit)
            ->getQuery()
            ->getResult()
    ...
    public function findAllWithoutStartedElection(Designation $designation, int $offset = 0, int $limit = 200): array
    {
        $qb = $this->createQueryBuilder('c')
            ...
            ->andWhere('c.status = :status')
            ->setParameters(new ArrayCollection([
                new Parameter('status', Committee::APPROVED),
                new Parameter('date', $designation->getCandidacyStartDate()),
            ]))
            ->setFirstResult($offset)
            ->setMaxResults($limit)
            ->groupBy('c.id')
    ...
    public function findCommitteeForRecentCandidate(Designation $designation, Adherent $adherent): ?Committee
    {
        return $this->createQueryBuilder('committee')
            ...
            ->setParameters(new ArrayCollection([
                new Parameter('designation_type', $designation->getType()),
                new Parameter('candidacy_status', CandidacyInterface::STATUS_CONFIRMED),
                new Parameter('adherent', $adherent),
                new Parameter('candidacy_date', new \DateTime('-3 months')),
                new Parameter('status', Committee::APPROVED),
            ]))
            ->setMaxResults(1)
            ->getQuery()
    ...
    public function findCommitteeForRecentVote(Designation $designation, Adherent $adherent): ?Committee
    {
        return $this->createQueryBuilder('committee')
            ...
            ->setParameters(new ArrayCollection([
                new Parameter('designation_type', $designation->getType()),
                new Parameter('adherent', $adherent),
                new Parameter('vote_date', new \DateTime('-3 months')),
                new Parameter('status', Committee::APPROVED),
            ]))
            ->setMaxResults(1)
            ->getQuery()
    ...
    public function getCommitteesPerimeters(): array
    {
        $rows = $this->createQueryBuilder('c')
            ...
            ->setParameters(new ArrayCollection([
                new Parameter('city', Zone::CITY),
                new Parameter('city_child_types', [Zone::CANTON, Zone::CITY_COMMUNITY]),
            ]))
            ->getQuery()
