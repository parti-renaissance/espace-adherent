...
class ZoneRepository extends ServiceEntityRepository
{
    ...
    public function findOneByPostalCode(string $postalCode): ?Zone
    {
        return $this->createQueryBuilder('zone')
            ...
            ->setParameters(new ArrayCollection([
                new Parameter('postal_code_1', $postalCode.'%'),
                new Parameter('postal_code_2', '%,'.$postalCode.'%'),
                new Parameter('zone_types', [Zone::CITY, Zone::BOROUGH]),
            ]))
            ->setMaxResults(1)
            ->getQuery()
            ->getOneOrNullResult()
    ...
    public function findOneDepartmentByPostalCode(string $postalCode): ?Zone
    {
        return $this->createQueryBuilder('dpt_zone')
            ...
            ->setParameters(new ArrayCollection([
                new Parameter('postal_code_1', $postalCode.'%'),
                new Parameter('postal_code_2', '%,'.$postalCode.'%'),
                new Parameter('dpt_zone_type', Zone::DEPARTMENT),
                new Parameter('city_zone_type', [Zone::CITY, Zone::BOROUGH]),
            ]))
            ->setMaxResults(1)
            ->getQuery()
            ->getOneOrNullResult()
    ...
    public function findGeoZoneByGeoRegion(Region $region): ?Zone
    {
        return $this->createQueryBuilder('zone')
            ...
            ->setParameters(new ArrayCollection([
                new Parameter('code', $region->getCode()),
                new Parameter('type', Zone::REGION),
            ]))
            ->setMaxResults(1)
            ->getQuery()
            ->getOneOrNullResult()
    ...
    public function findParent(string $parentType, string $childCode, string $childType): array
    {
        return $this->createQueryBuilder('parent_zone')
            ...
            ->setParameters(new ArrayCollection([
                new Parameter('child_zone_type', $childType),
                new Parameter('child_zone_code', $childCode),
                new Parameter('parent_zone_type', $parentType),
            ]))
            ->getQuery()
            ->getResult()
        ;
    ...
    public function getAllForAdherentsStats(): array
    {
        return $this->createQueryBuilder('zone')
            ...
            ->setParameter('assembly', '%'.ZoneTagEnum::ASSEMBLY.'%')
            ->getQuery()
            ->getResult()
        ;
    }
