...
class ConfigureCommand extends Command
{
    ...
    private function configureLocalPoll(Designation $designation): void
    {
        ...
        foreach ($designation->poll->getQuestions() as $question) {
            $election->getCurrentRound()->addElectionPool($pool = new ElectionPool($question->content, $question->description, $question->isSeparator));
            $election->addElectionPool($pool);
    ...
    {
        return new Election(
            $this->entityManager->getReference(Designation::class, $designation->getId()),
            null,
            [new ElectionRound()],
            $electionEntity
        );
