        ...
        do {
            ...
            foreach ($paginator->getIterator() as $membership) {
                switch ($type) {
                    ...
                    case self::CP_TYPE: $object = $membership->getCitizenProject(); break;
                }

                if (!\in_array($object->getUuid()->toString(), $this->cacheUsedUuids)) {
                    $this->bus->dispatch(new CreateStaticSegmentCommand($object->getUuid(), \get_class($object)));
                    $this->cacheUsedUuids[] = $object->getUuid()->toString();
                }
        ...
        switch ($type) {
            ...
            case self::CP_TYPE:
                return $this->entityManager
                    ...
                    ->createQueryBuilder('membership')
                    ->addSelect('PARTIAL adherent.{id, uuid}, PARTIAL cp.{id, uuid}')
                    ->innerJoin('membership.adherent', 'adherent')
