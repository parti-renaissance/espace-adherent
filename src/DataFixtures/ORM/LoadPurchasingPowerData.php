...
class LoadPurchasingPowerData implements FixtureInterface
{
    ...
    {
        $interactive = self::createInteractive();
        $manager->persist($interactive);
        $manager->flush();

        $choices = self::createChoices($interactive);
    ...
    public static function createInteractive(): Interactive
    {
        $interactive = new Interactive();
        $interactive->setSlug('ton-pouvoir-achat');
        $interactive->setMeta('');
        $interactive->setPicture('');
        $interactive->setTitle('');
        $interactive->setSubtitle('');
        $interactive->setUUID(Uuid::fromString('0fd94563-7dbf-4892-90e7-a19d23fa4435'));

        return $interactive;
    }

    public static function createChoices(Interactive $interactive = null): array
    {
        // Step 0
        $choices[] = self::createNoStepChoice(
            '28ceb6d3-ec64-4a58-99a4-71357600d07c',
            InteractiveChoice::MAIL_INTRODUCTION_KEY,
            'Introduction',
        ...
        $choices[] = self::createNoStepChoice(
            ...
            InteractiveChoice::MAIL_CONCLUSION_KEY,
            'Conclusion',
        ...
        $choices[] = self::createNoStepChoice(
            ...
            InteractiveChoice::MAIL_COMMON_KEY,
            'Mesures communes',
    ...
    private static function createNoStepChoice(string $uuid, string $key, string $label, string $content, Interactive $interactive = null): InteractiveChoice
    {
        return static::createChoice($uuid, 0, $key, $label, $content, $interactive);
    }

    private static function createChoice(string $uuid, int $step, string $key, string $label, string $content, Interactive $interactive = null): InteractiveChoice
    {
        return new InteractiveChoice(Uuid::fromString($uuid), $step, $key, $label, $content, $interactive);
    }

    private static function createFirstStepChoice(string $uuid, string $key, string $label, string $content, Interactive $interactive = null): InteractiveChoice
    {
        return static::createChoice($uuid, 1, $key, $label, $content, $interactive);
    }

    private static function createSecondStepChoice(string $uuid, string $key, string $label, string $content, Interactive $interactive = null): InteractiveChoice
    {
        return static::createChoice($uuid, 2, $key, $label, $content, $interactive);
    }

    private static function createThirdStepChoice(string $uuid, string $key, string $label, string $content, Interactive $interactive = null): InteractiveChoice
    {
        return static::createChoice($uuid, 3, $key, $label, $content, $interactive);
    }
}
