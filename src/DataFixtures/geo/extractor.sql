...
FROM enmarche.geo_region grINNER JOIN enmarche.geo_country gc ON gc.id = gr.country_id
;
...
INNER JOIN enmarche.geo_region gr ON gr.id = gd.region_id
WHERE gr.name IN ('Île-de-France', 'Provence-Alpes-Côte d''Azur', 'Auvergne-Rhône-Alpes')
;

-- 07-city-community.csv
SELECT gcc.id, gcc.code, gcc.name, GROUP_CONCAT(gd.code) AS code_department
...
WHERE gc.active = TRUE AND gr.name IN ('Île-de-France', 'Provence-Alpes-Côte d''Azur', 'Auvergne-Rhône-Alpes')
