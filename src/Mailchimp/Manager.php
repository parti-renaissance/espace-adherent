...
class Manager implements LoggerAwareInterface
{
    ...
    public function getContent(AdherentMessageInterface $message): string
    {
    ...
    public function editCampaign(AdherentMessageInterface $message): bool
    {
        ...
        if (!$campaignId = $message->getExternalId()) {
            if (!$campaignId = $this->driver->createCampaign($editCampaignRequest)) {
                throw new \RuntimeException(sprintf('Campaign for the message "%s" was not created', $message->getUuid()));
            }
    ...
    {
        if (!$message->getExternalId()) {
            throw new InvalidCampaignIdException(sprintf('Message "%s" does not have a valid campaign id', $message->getUuid()->toString()));
        }

        $requestBuilder = $this->requestBuildersLocator->get(CampaignRequestBuilder::class);

        if (!$this->driver->editCampaignContent($message->getExternalId(), $requestBuilder->createContentRequest($message))) {
            $this->logger->warning(sprintf('Campaign content of "%s" message was not modified', $message->getUuid()->toString()));
    ...
    public function deleteCampaign(string $campaignId): void
    {
        if (!$this->driver->deleteCampaign($campaignId)) {
            $this->logger->warning(sprintf('Campaign "%s" was not be deleted', $campaignId));
        }
    }
}
