...
CKEDITOR.dialog.add("specialchar",(k) => {
 let e; const n = k.lang.specialchar; const m = function (c) { let b; c = c.data ? c.data.getTarget() : new CKEDITOR.dom.element(c); "a" == c.getName() && (b = c.getChild(0).getHtml()) && (c.removeClass("cke_light_background"),e.hide(),c = k.document.createElement("span"),c.setHtml(b),k.insertText(c.getText())); }; const p = CKEDITOR.tools.addFunction(m); let l; const g = function (c,b) {
 let a; b = b || c.data.getTarget(); "span" == b.getName() && (b = b.getParent()); if ("a" == b.getName() && (a = b.getChild(0).getHtml())) {
 l && d(null,l);
const f = e.getContentElement("info","htmlPreview").getElement(); e.getContentElement("info","charPreview").getElement().setHtml(a); f.setHtml(CKEDITOR.tools.htmlEncode(a)); b.getParent().addClass("cke_light_background"); l = b;
}
}; var d = function (c,b) {
...
l = void 0);
}; const q = CKEDITOR.tools.addFunction((c) => {
 c = new CKEDITOR.dom.event(c); const b = c.getTarget(); let a; a = c.getKeystroke(); const f = "rtl" == k.lang.dir; switch (a) {
...
onMouseover: g,
onMouseout: d,
focus() { const c = this.getElement().getElementsByTag("a").getItem(0); setTimeout(() => { c.focus(); g(null,c); },0); },
onShow() {
 const c = this.getElement().getChild([0,
0,0,0,0]); setTimeout(() => { c.focus(); g(null,c); },0);
},
onLoad(c) { e = c.sender; } },{ type: "hbox",
align: "top",
widths: ["100%"],
children: [{ type: "vbox",
align: "top",
children: [{ type: "html",html: "\x3cdiv\x3e\x3c/div\x3e" },{ type: "html",id: "charPreview",className: "cke_dark_background",style: "border:1px solid #eeeeee;font-size:28px;height:40px;width:70px;padding-top:9px;font-family:'Microsoft Sans Serif',Arial,Helvetica,Verdana;text-align:center;",html: "\x3cdiv\x3e\x26nbsp;\x3c/div\x3e" },{ type: "html",
id: "htmlPreview",
