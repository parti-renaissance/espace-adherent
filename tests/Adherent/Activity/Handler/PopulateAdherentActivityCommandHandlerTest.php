...
use App\Adherent\Activity\Handler\PopulateAdherentActivityCommandHandler;
...
class PopulateAdherentActivityCommandHandlerTest extends AbstractKernelTestCase
{
    ...
    public function testInsertsActionHistoryRows(): void
    {
        ...
        $results = $this->connection->executeQuery(
            'SELECT * FROM adherent_activity WHERE source_type = :sourceType AND adherent_id = :adherentId',
    ...
    public function testFiltersHitsByEventType(): void
    {
        ...
        $presentCount = (int) $this->connection->executeQuery(
            'SELECT COUNT(*) FROM adherent_activity WHERE source_type = :sourceType AND source_id IN (:sourceIds)',
        ...
        $impressionCount = (int) $this->connection->executeQuery(
            'SELECT COUNT(*) FROM adherent_activity WHERE source_type = :sourceType AND source_id = :sourceId',
    ...
    public function testMapsHitFieldsToMetadata(): void
    {
        ...
        $row = $this->connection->executeQuery(
            'SELECT metadata FROM adherent_activity WHERE source_type = :sourceType AND source_id = :sourceId',
    ...
    public function testIsIdempotent(): void
    {
        ...
        ($this->handler)(new PopulateAdherentActivityCommand(SourceTypeEnum::ActionHistory));
        ($this->handler)(new PopulateAdherentActivityCommand(SourceTypeEnum::Hit));
        $countAfterFirst = (int) $this->connection->fetchOne('SELECT COUNT(*) FROM adherent_activity');

        ($this->handler)(new PopulateAdherentActivityCommand(SourceTypeEnum::ActionHistory));
        ($this->handler)(new PopulateAdherentActivityCommand(SourceTypeEnum::Hit));
        $countAfterSecond = (int) $this->connection->fetchOne('SELECT COUNT(*) FROM adherent_activity');
    ...
    protected function setUp(): void
    {
        ...
        $this->handler = new PopulateAdherentActivityCommandHandler($this->connection, $this->bus);

        $this->adherent = $this->createAdherent('test.activity@example.com');
        $this->manager->persist($this->adherent);
        $this->manager->flush();
    }
}
