...
class AdminTimelineMeasureControllerTest extends MysqlWebTestCase
{
    ...
    {
        /* @var $measure Measure */
        $measure = $this->measureRepository->findOneByTitle(LoadTimelineData::MEASURES['TM001']['title']);
        $currentTheme = $this->themeRepository->findOneByTitle(LoadTimelineData::THEMES['TT001']['title']);
        $newTheme = $this->themeRepository->findOneByTitle(LoadTimelineData::THEMES['TT003']['title']);
        ...
        $indexedEntities = $this->client->getContainer()->get(ManualIndexer::class)->getLastIndex();
        ...
        $this->assertSame(LoadTimelineData::MEASURES['TM001']['title'], $measure->getTitle());

        /* @var $oldTheme Theme */
        $oldTheme = $indexedEntities[1];
        $this->assertInstanceOf(Theme::class, $oldTheme);
        $this->assertSame(LoadTimelineData::THEMES['TT001']['title'], $oldTheme->getTitle());

        /* @var $currentTheme Theme */
        $currentTheme = $indexedEntities[2];
        $this->assertInstanceOf(Theme::class, $currentTheme);
        $this->assertSame(LoadTimelineData::THEMES['TT003']['title'], $currentTheme->getTitle());
    }
