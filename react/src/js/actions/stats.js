...
export function getAdherentsStats(query) {
    return {
        ...
        payload: Promise.all([
            callApi(`/api/adherents/count`),
            callApi(`/api/adherents/count-by-referent-area`)
        ]).then(([adherentsCount, adherents]) => ({
            adherentsCount,
            adherents
        }))
    };
...
export function getEventsStats(query) {
    return {
        type: EVENTS_STATS,
        payload: Promise.all([
            callApi(`/api/events/count`),
            callApi(`/api/events/count-participants`)
        ]).then(([eventsCount, participantsCount]) => ({
            eventsCount,
            participantsCount
        }))
    };
