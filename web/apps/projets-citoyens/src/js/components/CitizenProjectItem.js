...
const CitizenProjectItem = (props) => {
    const { thumbnail, title, subtitle, author, localisation, url, district } = props;

    return (
        <a href={url} className="citizen__project__item" target="_blank">
            ...
            <div className="citizen__project__item__content">
                ...
                <div className="citizen__project__item__footer">
                    Par <span className="citizen__project__item__author">{author}</span> à <span>{localisation}</span>
                </div>
            </div>
        </a>
    );
